/**
 * Tensorix Provider Extension
 *
 * Registers Tensorix (tensorix.ai) as a custom provider using the openai-completions API.
 * Base URL: https://api.tensorix.ai/v1
 *
 * Tensorix proxies multiple model families (DeepSeek, GLM, Kimi, Qwen, MiniMax,
 * Llama, Mistral, GPT-OSS, Nemotron) through an OpenAI-compatible API. The
 * /v1/models endpoint returns minimal metadata (id only — no pricing, context
 * lengths, or reasoning flags). The patch.json file provides these based on
 ...
 *   - All models are currently free during preview
 *
 ...
 *   export TENSORIX_API_KEY=your-api-key
 *
 *   # Run pi with the extension
 *   pi -e /path/to/pi-tensorix-provider
 *
 * Then use /model to select from available models like DeepSeek V4 Pro,
 * GLM 5.1, Kimi K2.6, Qwen3.5, and more.
 *
...
/** Transform a model from the Tensorix /v1/models API. */
...
async function resolveApiKey(modelRegistry: ModelRegistry): Promise<void> {
  cachedApiKey = await modelRegistry.getApiKeyForProvider("tensorix") ?? undefined;
}
